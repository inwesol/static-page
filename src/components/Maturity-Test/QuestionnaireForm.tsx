...
const QuestionnaireForm: React.FC = () => {
  ...
  useEffect(() => {
    const allQuestionsAnswered = QUESTIONS.every(
      (_, index) => responses[index + 1] !== undefined
    );
  ...
  return (
    <Card>
      ...
      <h2 className="text-2xl font-semibold mb-6 " style={{ color: "#00B24B" }}>
        Career Maturity Inventory
      </h2>
      <p className="mb-6 text-gray-700">
        There are 24 statements about choosing the kind of job or work that you
        will probably do when you finish school. Read each statement and select
        ...
        <div className="space-y-6">
          {currentQuestions.map((question, index) => {
            ...
            return (
              <div
                key={questionNumber}
                className="p-5 bg-white rounded-lg shadow-sm border border-gray-100 hover:shadow-md transition-all duration-200"
              >
                ...
                <div className="flex space-x-3 mt-2">
                  <button
                    type="button"
                    onClick={() =>
                      handleResponseChange(questionNumber, "Agree")
                    }
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      handleResponseChange(questionNumber, "Disagree")
                    }
                    className={`py-2 px-6 rounded-full text-sm font-medium transition-all duration-200 flex-1 ${
